...
# Test the agent with a goal description for one of the tools
# Examples: "make the image grayscale", "detect edges in the picture", "blur the photo", "resize the image to a small size"
goal_descriptions = [
    "detect edges in the picture"
]

for goal_description in goal_descriptions:
    result = agent.run("kento-nanami-jujutsu-kaisen-4k-wallpaper-uhdpaper.com-544@0@e.jpg", goal_description)

if result is not None:
    result.show()
else:
    print(f"Agent run failed for goal: {goal_description}")
